...
function check_conflict($conn, $day, $start_time, $end_time, $teacher_id, $group_id, $exclude_id = 0) {
    $sql = "SELECT COUNT(timetable_id) as conflict_count FROM timetable 
            ...
            AND end_time > ?
            AND timetable_id != ?
            AND is_active = 1"; // On ne vérifie les conflits que sur les séances actives
 ...
 * La séance n'est pas réellement supprimée, mais marquée comme inactive.
 */
function handle_delete($conn) {
    ...
    // LOGIQUE DE SOFT DELETE : On met à jour la colonne is_active à 0
    $sql = "UPDATE timetable SET is_active = 0 WHERE timetable_id = ?";
    
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, "i", $_POST['timetable_id']);

    if (mysqli_stmt_execute($stmt)) {
        // Le message est adapté pour refléter l'action d'annulation
